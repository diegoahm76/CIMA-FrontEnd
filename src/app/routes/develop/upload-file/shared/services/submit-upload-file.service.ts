...
export interface SubmitUploadFileState {
  status: StoreStatus;
...
@Injectable()
export class SubmitUploadFileService extends Store<
  StreamInput,
...
> {
      ...
      switchMap(({status, data, oldModel}: StreamInput) => {
        ...
        return concat(
          ...
          forkJoin([this.doQuery(data), timer(environment.minDelay)]).pipe(
            map(([state, dummy]: [SubmitUploadFileState, any]) => {
  ...
  private doQuery(data: Params): Observable<SubmitUploadFileState> {
    return this.apiService.save('users', data).pipe(
      ...
      map((model: DevelopUserModel) => {
        return {status: StoreStatus.OK, model} as SubmitUploadFileState;
      }),
      catchError((error: ApplicationError | any) => {
        return of({status: StoreStatus.ERROR, error} as SubmitUploadFileState);
      })
    );
  }
